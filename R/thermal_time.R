...
#' [APSIM Next Generation fababean Documentation](https://notes.apsimng.bangyou.me/docs/Plants/fababean/Phenology/ThermalTime.html).
#' 
#' @param maxt The maximum temperature
#' @param mint The minimum temperature
#' @param x_temp The cardinal temperatures using to calculate thermal time. The default values are c(2, 30, 35) defined in fababean$get("phenology.thermal_time.x").
#' @param y_temp The effective thermal time at the corresponding cardinal temperatures. The default values are c(0, 28, 0) defined in fababean$get("phenology.thermal_time.y").
...
thermal_time <- function(mint, 
    maxt,
    x_temp = fababean$get("phenology.thermal_time.x"), 
    y_temp = fababean$get("phenology.thermal_time.y"),
    method = fababean$get("phenology.thermal_time.method"),
    ...) {
